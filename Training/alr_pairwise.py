...
class TrainModel:
    ...
    def train_next_decoder_pair(self):
        """
        Enable training for the current decoder pair: (current_layer, current_layer + 1).
        """
        next_layer = self.current_layer + 1

        for i, layer in enumerate(self.model.decoder.layers):
            requires_grad = i == self.current_layer or i == next_layer
            for param in layer.parameters():
                param.requires_grad = requires_grad

        logger.info(f"Training decoder pair: ({self.current_layer}, {next_layer})")

    def switch_pair(self):
        """
        Move to the next overlapping decoder pair: (0,1) → (1,2) → ... → (4,5) → (0,1) ...
        """
        ...
        logger.info(f"Switched to decoder layer pair starting at {self.current_layer}")
    ...
    def forward(self, num_epochs=30):
        ...
        for epoch in range(num_epochs):
            ...
            # Step scheduler, switch pair and clean cache
            # self.scheduler.step()
